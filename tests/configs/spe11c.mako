...
flow = "flow --newton-min-iterations=1 --enable-opm-rst-file=true --output-extra-convergence-info=steps,iterations"
...
x_n = [20] # If cartesian, number of x cells [-]; otherwise, variable array of x-refinement
y_n = [3] # If cartesian, number of y cells [-]; otherwise, variable array of y-refinement [-] (for spe11c)
z_n = [10] # If cartesian, number of z cells [-]; if tensor, variable array of z-refinement; if corner-point, fix array of z-refinement (11 or 18 entries)
temperature = [70, 36.12] # Temperature bottom and top rig [C]
datum = 300 # Datum [m]
...
inj = [[1, 1, 1, 50, 10, 1, 0, 10, "10 100 4* 0.2 / 6* 1e-5 / 2* 12"]]
