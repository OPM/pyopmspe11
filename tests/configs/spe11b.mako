...
flow = "flow --newton-min-iterations=1 --enable-opm-rst-file=true --output-extra-convergence-info=steps,iterations --enable-well-operability-check=false --min-time-step-before-shutting-problematic-wells-in-days=1e-99"

# Set the model parameters
spe11 = "spe11b" # Name of the spe case (spe11a, spe11b, or spe11c)
version = "release" # OPM Flow version (release or master)
model = "${model}" # Name of the co2 model (immiscible, isothermal, convective, isothermal, or complete)
grid = "tensor" # Type of grid (cartesian, tensor, or corner-point)
dims = [8400.0, 1.0, 1200.0] # Length, width, and depth [m]
x_n = [2, 5, 8, 8, 8, 5, 2] # If cartesian, number of x cells [-]; otherwise, variable array of x-refinement
y_n = [1] # If cartesian, number of y cells [-]; otherwise, variable array of y-refinement [-] (for spe11c)
z_n = [2, 2, 1, 1, 2, 1, 3, 2, 6, 2, 3, 2, 2, 8, 4, 8, 8, 1] # If cartesian, number of z cells [-]; if tensor, variable array of z-refinement; if corner-point, fix array of z-refinement (11 or 18 entries)
...
s_w = "1 - np.linspace(0, 1.0, npoints)"                                                                 # Points to evaluate the saturation functions (s_w) [-]
