...
${dic['wellijk'][i][0]} ${dic['wellijk'][i][1]+k} ${dic['wellijk'][i][2] if i==1 else dic['wellkh'][k]} ${'GAS' if dic['inj'][j][2+3*i] > 0 else 'WATER'} ${f"{dic['inj'][j][3+3*i] * 86400. / (1+dic['wellijkf'][i][1]-dic['wellijk'][i][1]):E}"} ${f"1* {dic['inj'][j][4+3*i]:E} " if dic['model'] in ["complete", "convective"] else ""}/
% endfor
% endif
% if dic["lower"]:
<% break %>
