...
def create_corner_point_grid(dic, xcoord, zcoord):
    ...
    grid.append("-- Copyright (C) 2023-2025 NORCE Research AS\n")
    grid.append("COORD\n")
    for j in range(dic["noCells"][1] + 1):
        for i in range(dic["noCells"][0] + 1):
            tmp.append(
                f"{xcoord[i * (dic['noCells'][2] + 1)]:E} {dic['ymy'][j]:E} 0 "
                + f"{xcoord[i * (dic['noCells'][2] + 1) + dic['noCells'][2]]:E} "
                + f"{dic['ymy'][j]:E} 0 "
            )
    ...
    if dic["spe11"] == "spe11c":
        ...
        for k in range(dic["noCells"][2] - 1):
            ...
            tmp.extend(tmp[-2 * dic["noCells"][1] * dic["noCells"][0] :])
        for j in range(dic["noCells"][1]):
            for i in range(dic["noCells"][0]):
                tmp.append(
...
def write_keywords(dic):
    ...
    for names in keywords:
        ...
        dic[f"{names}"].insert(0, "-- Copyright (C) 2023-2025 NORCE Research AS\n")
        dic[f"{names}"].append("/")
        with open(
...
def added_pv(dic, git):
    ...
    dic["porv"].insert(0, "-- Copyright (C) 2023-2025 NORCE Research AS")
    dic["porv"].append("/")
    with open(
