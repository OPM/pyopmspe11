...
def grid(dic):
    ...
    if dic["grid"] != "corner-point":
        if (dic["csp11"] == "csp11b" or dic["csp11"] == "csp11c") and 1.1 * dic[
            "widthBuffer"
        ] < dic["xmx"][1]:
    ...
    if dic["csp11"] == "csp11a":
        ...
        for k in range(dic["noCells"][2]):
            for i in range(dic["noCells"][0]):
                ...
                if i == 0 and (
                    int(dic["ids_gmsh"][fgl][0]) != 1
                    and int(dic["ids_gmsh"][fgl][0]) != 7
                ):
                    dic["poro"].append(
                        f"{dic['pvAdded']*dic['ymy'][1]*dic['dz'][k] :E}"
                    )
                    ...
                    dic["poro"].append(dic["rock"][int(dic["ids_gmsh"][fgl][0]) - 1][1])
            for _ in range(dic["noCells"][1] - 1):
...
def corner_point_handling(dic):
    ...
    """
    well1, well2 = [], []
    gridf = EclGrid(f"{dic['exe']}/{dic['fol']}/preprocessing/INITIAL.EGRID")
    ...
    if dic["csp11"] == "csp11a":
        ...
        dic["wellijk"][0] = [gridf.cell(idwell1).i + 1, 1, gridf.cell(idwell1).k + 1]
        dic["wellijk"][1] = [gridf.cell(idwell2).i + 1, 1, gridf.cell(idwell2).k + 1]
        # Work in process to implement properly this for the corner-point grid in csp11c
        if dic["csp11"] == "csp11c":
            ...
            dic["ymy_center"] = (
                np.array(dic["ymy"][1:]) + np.array(dic["ymy"][:-1])
            ) / 2.0
            wji = (
                pd.Series(abs(dic["wellCoord"][0][1] - dic["ymy_center"])).argmin() + 1
            )
            wjf = (
                pd.Series(abs(dic["wellCoordf"][0][1] - dic["ymy_center"])).argmin() + 1
            )
            dic["wellijk"][0][1] = wji
            dic["wellijk"][1][1] = wji
...
def positions(dic):
    ...
    for names in ["satnum", "poro", "permx", "thconr"]:
        dic[f"{names}"] = []
    if dic["grid"] == "corner-point":
        dic = corner_point_handling(dic)
    else:
        dic = structured_handling(dic)
    return dic
...
def corner(dic):
    """
    Function to create a csp11 corner-point grid
